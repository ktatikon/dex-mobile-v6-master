...
class KYCService {
  constructor() {
    // IDfy API Configuration with CORRECT base URL from Flutter documentation
    ...
    // IDfy API Headers Configuration (CORRECT format from documentation)
    this.headers = {
      'api-key': this.apiKey,
  ...
  async sendAadhaarOtp(aadhaarNumber, userId) {
    try {
      ...
      // Call IDfy API endpoint for Aadhaar OTP initiation
      // Based on research, trying the most likely endpoint patterns
  ...
  async verifyAadhaarOtp(taskId, otp, userId) {
    try {
      ...
      // IDfy API payload for OTP verification (CORRECTED FORMAT based on research)
      // Based on research, OTP verification might need different structure
      const payload = {
        task_id: taskId,
        group_id: taskData.group_id || `group_${Date.now()}`, // Use original group_id if available
        data: {
          code: otp
        }
      };
      ...
      // Use the same endpoint pattern discovery as OTP initiation
      const possibleEndpoints = [
        '/v3/tasks/async/verify_with_source/ind_aadhaar_otp',
        '/v3/tasks/sync/verify_with_source/ind_aadhaar_otp',
        '/v3/tasks/async/verify_with_source/aadhaar_otp',
      ...
      for (const endpoint of possibleEndpoints) {
        try {
          response = await this.axiosInstance.post(endpoint, payload);
          logger.debug(`Aadhaar OTP verification endpoint found: ${endpoint}`);
          break;
        } catch (error) {
          lastError = error;
          if (error.response?.status !== 404) {
            // If it's not a 404, this might be the right endpoint
            break;
          }
        }
      }
  ...
  async verifyPAN(panNumber, userId) {
    try {
      ...
      // IDfy API payload for PAN verification (CORRECTED FORMAT based on research)
      // Based on GitHub repository analysis, IDfy expects this specific structure
      const payload = {
      ...
      // Call IDfy API endpoint for PAN verification
  ...
  async verifyPassport(passportNumber, dob, userId) {
    try {
      ...
      // IDfy API payload for Passport verification (CORRECTED FORMAT based on research)
      // Based on GitHub repository analysis, IDfy expects this specific structure
      const payload = {
      ...
      // Call IDfy API endpoint for Passport verification
      const response = await this.axiosInstance.post(
