...
CREATE TABLE IF NOT EXISTS public.risk_assessments (
    ...
    risk_level risk_category_enum NOT NULL,
    
    -- Risk Factors (Flexible JSON structure)
    ...
    assessment_method VARCHAR(20) DEFAULT 'automated', -- automated, manual, hybrid
    assessment_trigger VARCHAR(50) -- onboarding, periodic, transaction, etc.
);
...
CREATE OR REPLACE FUNCTION public.get_kyc_completion_status(user_uuid UUID)
RETURNS TABLE (
...
BEGIN
    RETURN QUERY
    SELECT 
        COALESCE(kup.kyc_completion_percentage, 0) as completion_percentage,
        ARRAY[]::TEXT[] as missing_documents, -- Implement logic based on requirements
        ARRAY[]::TEXT[] as next_steps -- Implement logic based on requirements
    FROM public.kyc_user_profiles kup
    WHERE kup.user_id = user_uuid;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to update KYC completion percentage
CREATE OR REPLACE FUNCTION public.update_kyc_completion()
RETURNS TRIGGER AS $$
BEGIN
    -- Calculate completion percentage based on verified documents
    UPDATE public.kyc_user_profiles 
    SET 
        kyc_completion_percentage = (
            (CASE WHEN aadhaar_verified THEN 40 ELSE 0 END) +
            (CASE WHEN pan_verified THEN 30 ELSE 0 END) +
            (CASE WHEN passport_verified THEN 20 ELSE 0 END) +
            (CASE WHEN address_verified THEN 10 ELSE 0 END)
        ),
        last_updated_at = NOW(),
        kyc_status = CASE 
            WHEN (aadhaar_verified AND pan_verified) THEN 'verified'::kyc_status_enum
            WHEN (aadhaar_verified OR pan_verified) THEN 'in_progress'::kyc_status_enum
...
-- Triggers to automatically update KYC completion
CREATE TRIGGER update_kyc_completion_aadhaar
    AFTER UPDATE ON public.aadhaar_verifications
    FOR EACH ROW
    WHEN (NEW.verification_status = 'verified')
    EXECUTE FUNCTION public.update_kyc_completion();

CREATE TRIGGER update_kyc_completion_pan
    AFTER UPDATE ON public.pan_verifications
    FOR EACH ROW
    WHEN (NEW.verification_status = 'verified')
    EXECUTE FUNCTION public.update_kyc_completion();
