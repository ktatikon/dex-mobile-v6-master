...
class AuthMiddleware {
  ...
  authenticate(req, res, next) {
    try {
      ...
      if (apiKey) {
        ...
        req.user = {
          type: 'internal',
          permissions: ['aml:read', 'aml:write', 'aml:admin']
        };
  ...
  /**
   * Validate user context for AML operations
   */
  validateUserContext(req, res, next) {
    try {
      ...
      // Users can only access their own AML data
      if (userId && requestUserId && userId !== requestUserId) {
        ...
        return res.status(403).json({
          success: false,
          error: 'Access denied - Can only access own AML data'
        });
      }
